...
(async () => {
    ...
    if (firstday.valueOf() < Date.now()) { // req date < exec date
        ...
        if (todayDate.getUTCDay() === 0) { // its a monday, exec the testing info scraper
        ...
        var alamedaCases = await page.evaluate(() => {
            if (JSON.parse(document.querySelector("body > pre").innerText).result.records.length > 1) {
                for (x = 0; x < JSON.parse(document.querySelector("body > pre").innerText).result.records.length; x++) {
                    if (JSON.parse(document.querySelector("body > pre").innerText).result.records[0].date.includes(`${prevEntryDate.getFullYear()}-${("0" + (prevEntryDate.getUTCMonth() + 1)).slice(-2)}-${("0" + prevEntryDate.getUTCDate()).slice(-2)}`)) {
        ...
        if (config.lastAccessDataNotComplete) {
            var prevEntryDate = new Date(writtenWeeks[writtenWeeks.length - 2] + " 2021") // -2 because the last one is the one that will get deleted, and the request above is made using the last date in the array
        } else {
            var prevEntryDate = new Date(writtenWeeks[writtenWeeks.length - 1] + " 2021")
        }
        await page.goto(`https://data.ca.gov/api/3/action/datastore_search?resource_id=926fd08f-cc91-4828-af38-bd45de97f8c3&q={"county":"alameda","date":"${prevEntryDate.getFullYear()}-${("0" + (prevEntryDate.getUTCMonth() + 1)).slice(-2)}-${("0" + prevEntryDate.getUTCDate()).slice(-2)}"}`)
        await page.waitForTimeout(1000)
